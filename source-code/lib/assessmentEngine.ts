...
export function evaluateAssessment(
  ...
  learnerAnswers: Record<string, number>, // questionId -> selected option index
  ...
  // Update each evaluated competency using bounded formula:
  ...
  for (const [compId, stat] of Object.entries(compStats)) {
    ...
    const afterScore = Math.min(100, Math.max(0, Math.round(beforeScore * 0.7 + assessmentCompScore * 0.3)));
