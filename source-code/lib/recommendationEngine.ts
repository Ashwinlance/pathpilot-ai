...
export function getTrainingRecommendations(
  ...
  // Filtering logic
  if (filterCategory === 'igot') {
    courses = courses.filter(c => c.source.includes('iGOT'));
  } else if (filterCategory === 'nssta') {
    courses = courses.filter(c => c.source.includes('NSSTA'));
  } else if (filterCategory === 'high_priority') {
    courses = courses.filter(c => c.competencies.some(cid => gapsMap.get(cid)?.priority === 'High'));
  } else if (filterCategory === 'statistical') {
    courses = courses.filter(c => c.competencies.some(cid => cid.startsWith('stat_')));
  } else if (filterCategory === 'technical') {
    courses = courses.filter(c => c.competencies.some(cid => cid.startsWith('tech_')));
  } else if (filterCategory === 'governance') {
    courses = courses.filter(c => c.competencies.some(cid => cid.startsWith('gov_')));
  } else if (filterCategory === 'managerial') {
    courses = courses.filter(c => c.competencies.some(cid => cid.startsWith('mgt_')));
  }
  ...
  const recommendations: RecommendedCourse[] = courses.map(course => {
    ...
    // Evaluate matching competencies
    for (const compId of course.competencies) {
      const gapAnalysis = gapsMap.get(compId);
      if (gapAnalysis) {
        if (gapAnalysis.priority === 'High') {
          score += 50 + gapAnalysis.gap;
        } else if (gapAnalysis.priority === 'Medium') {
          score += 30 + gapAnalysis.gap;
        } else if (gapAnalysis.priority === 'Low') {
          score += 15 + gapAnalysis.gap;
        } else {
          score += 5;
        }

        if (gapAnalysis.gap > maxGapValue) {
          maxGapValue = gapAnalysis.gap;
    ...
    if (course.targetRoles.includes(targetRoleId)) {
      score += 20;
    }

    // Generate dynamic "Why Recommended" rationale
    let whyRecommended = '';
    ...
    if (primaryGap && primaryGap.gap > 0) {
      ...
      if (primaryGap.priority === 'High') {
        matchPriority = 'High Priority Match';
      } else {
      ...
      matchPriority = 'General Development';
    }

    return {
      course,
      relevanceScore: score,
      primaryGapAddressed: primaryGap,
      whyRecommended,
      matchPriority,
    };
  });
