...
export const actionLabels: Record<CopilotAction, string> = {
  ...
  repair: 'Repair prerequisite',
  next: 'Next step',
  explain_gap: 'Explain skill gap',
  why_recommended: 'Why recommended?',
  mistake: 'Explain my mistake',
  admin_top_gap: 'What is top workforce gap?',
  admin_dept: 'Department analysis',
  admin_rec: 'Executive recommendations',
};

export async function getLearningCopilotReply({
  message,
  action,
  context,
}: {
...
}): Promise<{ message: string; actions: CopilotAction[] }> {
  const lower = message.toLowerCase();

  // Admin Mode Responses
  if (context.userRole === 'admin' || action?.startsWith('admin_')) {
    if (action === 'admin_top_gap' || lower.includes('biggest') || lower.includes('top gap')) {
      ...
      return {
        message: `The top organizational skill gap across MoSPI workforce learners is ${topGap.competency} with an average gap of ${topGap.gap} points affecting ${topGap.affected} officers. Implementing targeted iGOT/NSSTA upskilling cohorts is recommended.`,
        actions: ['admin_rec', 'admin_dept', 'why_recommended'],
      };
    }

    if (action === 'admin_dept' || lower.includes('department')) {
      return {
        message: `Data Informatics & Innovation Division (DIID) and Survey Design & Research Division (SDRD) show high technical competency gaps in SQL and AI/ML. National Accounts Division leads in GDP & SNA 2008 compilation mastery.`,
        actions: ['admin_top_gap', 'admin_rec'],
      };
    }

    if (action === 'admin_rec' || lower.includes('recommendation') || lower.includes('prioritize')) {
      return {
        message: `Actionable Executive Recommendations:\n1. Deploy AI/ML e-learning cohort on iGOT Karmayogi (27pt deficit, 9 officers affected).\n2. Schedule NSSTA hands-on SQL query workshop for NAD & SDRD officers.\n3. Mandate 4-hour DPDP Act privacy module before statistical releases.`,
        actions: ['admin_top_gap', 'admin_dept'],
      };
    }
  }
  ...
  if (action === 'repair' || context.knowledgeGap) {
    return {
      ...
      actions: ['explain', 'example', 'repair'],
    };
  }

  if (action === 'mistake' || lower.includes('mistake') || lower.includes('score')) {
    return {
      message: context.latestAssessmentScore !== undefined
        ? `Your latest competency assessment score was ${context.latestAssessmentScore}%. Mistakes in specific question concepts indicate areas where reviewing prerequisite definitions (e.g. WHERE vs HAVING in SQL or Pandas DataFrames in Python) will improve your score.`
        : `Take the competency assessment generated from learning materials to receive detailed feedback on your mistakes and score impact.`,
      actions: ['explain', 'practice', 'next'],
    };
  }

  if (action === 'explain_gap' || lower.includes('gap')) {
    const topGap = context.topSkillGaps?.[0];
    if (topGap) {
      return {
        message: `Your current competency in ${topGap.competency} is lower than required for your target role (${context.targetRole ?? 'Target Role'}). Addressing a ${topGap.gap}-point gap in ${topGap.competency} is prioritized in your custom MoSPI learning roadmap.`,
        actions: ['why_recommended', 'practice', 'next'],
      };
    }
  }

  if (action === 'why_recommended' || lower.includes('recommend') || lower.includes('course')) {
    const topRec = context.recommendedCourses?.[0];
    if (topRec) {
      return {
        message: `The course "${topRec.title}" from ${topRec.provider} was recommended because: ${topRec.why}`,
        actions: ['explain', 'practice', 'next'],
      };
    }
    return {
      message: `Training courses are selected dynamically based on your MoSPI target role (${context.targetRole ?? 'Statistical Analyst'}). Courses targeting high-priority skill gaps (e.g. Python, SQL, Survey Design) appear at the top of your iGOT/NSSTA repository feed.`,
      actions: ['explain', 'practice', 'next'],
    };
  }
