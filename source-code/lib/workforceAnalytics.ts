...
export function getWorkforceAnalyticsSummary() {
  ...
  const domainTotals: Record<string, { demonstrated: number; count: number }> = { statistical: { demonstrated: 0, count: 0 }, technical: { demonstrated: 0, count: 0 }, governance: { demonstrated: 0, count: 0 }, managerial: { demonstrated: 0, count: 0 } };
  ...
  learners.forEach(learner => {
    ...
    for (const gap of gapOverview.gaps) {
      ...
      if (domainTotals[gap.domain]) { domainTotals[gap.domain].demonstrated += gap.currentLevel; domainTotals[gap.domain].count += 1; }
    }
  });
  ...
  const orgGaps = Object.entries(compTotals).map(([cid, data]) => {
    ...
    let priority = 'Low';
    if (gap >= 25 || (gap >= 15 && avgRequired >= 70)) priority = 'High'; else if (gap >= 10) priority = 'Medium';
  ...
  learners.forEach(l => { (deptMap[l.department] ??= []).push(l); });
  const departmentMetrics: DepartmentMetric[] = Object.entries(deptMap).map(([department, dLearners]) => {
    let dMatchSum = 0, dHoursSum = 0, dScoreSum = 0, dScoreCount = 0;
    dLearners.forEach(l => { const g = calculateSkillGaps(l.targetRole, l.competencies); dMatchSum += g.overallCompetencyMatch; dHoursSum += l.learningHours; l.assessmentScores.forEach(s => { dScoreSum += s; dScoreCount++; }); });
    const topGapForDept = topOrgSkillGaps[0];
    return { department, learnerCount: dLearners.length, avgCompetencyMatch: Math.round(dMatchSum / dLearners.length), avgLearningHours: Math.round(dHoursSum / dLearners.length), topGapCompetency: topGapForDept?.competencyName || 'SQL & Databases', topGapPoints: topGapForDept?.gap || 21, avgAssessmentScore: dScoreCount > 0 ? Math.round(dScoreSum / dScoreCount) : 75 };
  });
  const topGap = topOrgSkillGaps[0], secGap = topOrgSkillGaps[1];
  ...
  return { totalLearners, avgCompetencyMatch, avgLearningHours, avgAssessmentScore, activeHighPriorityGapsCount: topOrgSkillGaps.filter(g => g.priority === 'High').length, domainAverages: { statistical: domainTotals.statistical.count > 0 ? Math.round(domainTotals.statistical.demonstrated / domainTotals.statistical.count) : 70, technical: domainTotals.technical.count > 0 ? Math.round(domainTotals.technical.demonstrated / domainTotals.technical.count) : 58, governance: domainTotals.governance.count > 0 ? Math.round(domainTotals.governance.demonstrated / domainTotals.governance.count) : 64, managerial: domainTotals.managerial.count > 0 ? Math.round(domainTotals.managerial.demonstrated / domainTotals.managerial.count) : 75 }, topOrgSkillGaps, departmentMetrics, adminRecommendations };
}
