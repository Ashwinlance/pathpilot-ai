...
export type RoleGapOverview = {
  role: RoleDefinition;
  overallCompetencyMatch: number; // percentage (0-100)
  gaps: SkillGapAnalysis[];
  topPriorityGaps: SkillGapAnalysis[];
...
export function calculateSkillGaps(
  ...
  for (const [compId, requiredLevel] of Object.entries(requiredMap)) {
    ...
    if (gap === 0) {
      satisfiedCount++;
      priority = 'Satisfied';
      statusText = 'Role Requirement Met';
    } else if (gap >= 25 || (gap >= 15 && requiredLevel >= 70)) {
      priority = 'High';
      statusText = 'High Priority Gap';
    } else if (gap >= 10) {
      priority = 'Medium';
      statusText = 'Moderate Gap';
    } else {
      priority = 'Low';
    ...
    let explanation = '';
    if (gap > 0) {
      explanation = `${comp.name} is currently at ${currentLevel}%, while the selected ${role.title} role requires ${requiredLevel}%. The resulting ${gap}-point gap makes ${comp.name} a ${priority.toLowerCase()}-priority competency area.`;
    } else {
      explanation = `Your demonstrated mastery in ${comp.name} (${currentLevel}%) satisfies or exceeds the ${requiredLevel}% requirement for ${role.title}.`;
    }
  ...
  // Sort gaps by priority (High -> Medium -> Low -> Satisfied) and gap size
  ...
  const overallCompetencyMatch = totalRequiredSum > 0
    ? Math.round((totalDemonstratedSum / totalRequiredSum) * 100)
    : 100;
  ...
  return {
    role,
    overallCompetencyMatch,
    gaps,
    topPriorityGaps,
    satisfiedCount,
    totalRequiredCount: Object.keys(requiredMap).length,
  };
}
