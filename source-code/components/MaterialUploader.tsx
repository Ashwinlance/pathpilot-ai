...
export function MaterialUploader({
  ...
  return (
    <div className="space-y-6" data-testid="material-uploader">
      {/* Sample Preloaded Materials Selector */}
      <div>
          ...
          {SAMPLE_MATERIALS.map(mat => (
            <button
              ...
              className={cx(
                ...
                activeMaterial.title === mat.title
                  ? 'border-primary bg-primary/5 ring-1 ring-primary/30'
                  : 'border-border bg-card hover:bg-secondary/50'
              )}
            >
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary font-bold text-xs">
                {mat.fileType}
              </span>
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                  {mat.concepts.slice(0, 2).map(c => (
                    <span key={c} className="rounded-md bg-secondary px-2 py-0.5 text-[10px] font-semibold text-muted-foreground">
                      {c}
                    </span>
                  ))}
                </div>
      ...
      <div
        ...
        className={cx(
          'relative flex flex-col items-center justify-center rounded-2xl border-2 border-dashed p-8 text-center transition',
      ...
      >
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Drag and drop PDF, PPT, DOC, or TXT files here, or click to browse
        </p>
        <span className="mt-3 inline-flex rounded-full bg-secondary px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
          Supported: PDF • PPT • DOC • TXT
        </span>
      </div>

      {/* Extracted Concept & Status Summary */}
      <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <FileText size={18} className="text-primary" />
            <span className="font-serif font-bold text-sm">{activeMaterial.title}</span>
          </div>
          <span className={cx('inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider', status === 'Processed' ? 'bg-[hsl(155_32%_88%)] text-[hsl(162_46%_27%)]' : 'bg-secondary text-muted-foreground')}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 text-xs border-t border-border pt-3">
          <div>
            <span className="block font-bold uppercase tracking-wider text-[10px] text-muted-foreground mb-1">
              Extracted Concepts ({activeMaterial.concepts.length})
            </span>
            <div className="flex flex-wrap gap-1.5">
              {activeMaterial.concepts.map(c => (
                <span key={c} className="rounded-lg bg-primary/10 px-2.5 py-1 text-[11px] font-semibold text-primary">
                  {c}
                </span>
              ))}
            </div>
          ...
          <div>
            <span className="block font-bold uppercase tracking-wider text-[10px] text-muted-foreground mb-1">
              Detected Competencies ({activeMaterial.detectedCompetencies.length})
            </span>
            <div className="flex flex-wrap gap-1.5">
              {activeMaterial.detectedCompetencies.map(comp => (
                <span key={comp.id} className="rounded-lg bg-secondary px-2.5 py-1 text-[11px] font-semibold text-foreground border border-border">
                  {comp.name}
                </span>
              ))}
            </div>
