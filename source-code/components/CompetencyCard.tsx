        ...
        <div className="relative h-2.5 overflow-hidden rounded-full bg-secondary">
          ...
          <div
            className={cx(
              ...
              item.priority === 'High' && 'bg-accent',
              item.priority === 'Medium' && 'bg-[hsl(42_72%_55%)]',
              item.priority === 'Low' && 'bg-[hsl(197_45%_45%)]',
              item.priority === 'Satisfied' && 'bg-primary'
            )}
      ...
      <div className="mt-4 flex items-center justify-between border-t border-border/60 pt-3 text-xs">
        {item.gap > 0 ? (
          <span className="font-semibold text-accent">
            Competency Gap: <strong>{item.gap} points</strong>
          </span>
        ) : (
          <span className="font-semibold text-[hsl(162_46%_27%)]">
            ✓ Target Achieved ({item.currentLevel}%)
          </span>
        )}
