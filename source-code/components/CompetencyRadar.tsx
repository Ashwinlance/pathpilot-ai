  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-6 shadow-sm" data-testid="competency-radar">
      ...
      <div className="mt-6 space-y-4">
        {gaps.map(item => {
          const percentOfTarget = item.requiredLevel > 0
            ? Math.min(100, Math.round((item.currentLevel / item.requiredLevel) * 100))
            : 100;
            ...
            <div key={item.competencyId} className="space-y-1.5">
              <div className="flex flex-wrap items-center justify-between gap-1 text-xs">
                ...
                <span className="font-mono text-muted-foreground">
                  <strong className={item.gap > 0 ? 'text-accent' : 'text-primary'}>{item.currentLevel}%</strong> / {item.requiredLevel}%
              ...
              <div className="relative h-4 overflow-hidden rounded-md bg-secondary p-0.5">
                {/* Target marker line */}
                <div
                  className="absolute bottom-0 top-0 w-0.5 bg-foreground/60 z-20"
